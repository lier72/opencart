...
class info extends cdek_integrator {
	...
	public function getTariffMode() {
		return array(
			...
			3 => 'склад-дверь (С-Д)',
			4 => 'склад-склад (С-С)',
			5 => 'дверь-постамат (С-П)',
			6 => 'склад-постамат (С-П)'
		);
	}
	...
	public function getTariffList() {
		...
		return array(
			...
			'361' => array(
				'title'		=> 'Экспресс лайт (Д-П)',
				'mode_id'	=> 5,
			),
			'363' => array(
				'title'		=> 'Экспресс лайт (С-П)',
				'mode_id'	=> 6,
			),	
			'15' => array(
	...
	public function getInpostTariffs() {
		$postomats_id = array('301','302','361','363','366','368','376','378');
		return $postomats_id;
	}
