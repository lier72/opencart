...
class Server
{
        ...
        if (!$request_xml) {
            if (($query = $_SERVER['QUERY_STRING'])
                ...
                echo xmlrpc_encode_request(
                    null,
                    \Ripcord\Ripcord::fault(-1, 'No request xml found.'),
                    $this->outputOptions
                );
            }
        } else {
     ...
     *
     * @throws InvalidArgumentException (\Ripcord\Ripcord::cannotRecurse)  when passed a recursive multiCall
     * @throws BadMethodCallException   (\Ripcord\Ripcord::methodNotFound) when the requested method isn't available.
     *
     * @return mixed
     */
