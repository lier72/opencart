...
/*		'--db_hostname', 'localhost',
	...
	echo 'php cli_upgrade.php upgrade ' . $options . "\n\n";
}
/*
...
class ModelUpgradeCli extends Model{
    function upgrade()
    {
...
//   print_r(DIR_APPLICATION."\n");
        ...
        if (isset($files[0]))
            foreach ($files as $file) {
                ...
                try {
                    $this->load->model('upgrade/' . basename($file, '.php'));;
                    print_r("Calling 'model_upgrade_" . str_replace('.', '', basename($file, '.php')) . "->upgrade();' method \n");
                    // All upgrade methods require a upgrade method
                    $this->{'model_upgrade_' . str_replace('.', '', basename($file, '.php'))}->upgrade();

                } catch (Exception $exception) {
                    echo sprintf("OMG!!! Code: %u Message: '%s' in File %s line %u\n", $exception->getCode(), $exception->getMessage(), $exception->getFile(), $exception->getLine());
                }
            } else {
            echo 'Success!!!';
        }
...
$up = new ModelUpgradeCli($registry);
