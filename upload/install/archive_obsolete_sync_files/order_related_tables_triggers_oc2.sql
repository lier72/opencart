...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM ocus_order_id_map WHERE oc2_order_id = NEW.order_id AND sync_lock = 1) THEN
    SET should_sync = 0;
  END IF;
...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM ocus_order_id_map WHERE oc2_order_id = OLD.order_id AND sync_lock = 1) THEN
    SET should_sync = 0;
  END IF;
...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM ocus_order_id_map WHERE oc2_order_id = NEW.opencart_order_id AND sync_lock = 1) THEN
    SET should_sync = 0;
  END IF;
