-- ============================================
-- Order History Triggers for OC2
-- Syncs order_history table changes to OC3
-- ============================================
...
BEGIN
  ...
  IF should_sync = 1 THEN
    ...
    IF ROW_COUNT() = 0 THEN
      INSERT INTO `oc_order_sync_queue` (
        `source_db`, `table_name`, `operation`, `record_id`, `parent_order_id`,
        `data_json`, `sync_status`, `sync_ready`, `created_at`
      ) VALUES (
        current_source,
...
SELECT 'Order history triggers created for OC2' AS status;
