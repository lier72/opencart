...
/*  {"table":"ocus_banner_image","nf_old_table_fields":[],"nf_new_table_fields":{"2":"language_id","3":"title"}},*/
    $sql = "REPLACE INTO ocus_banner_image (`banner_image_id`, `banner_id`, `language_id`, `title`, `link`, `image`, `sort_order`) SELECT 
  a.banner_image_id, a.banner_id, b.language_id, b.title, a.link, a.image, a.sort_order  FROM `".OLD_DB_DATABASE."`.ocus_banner_image a 
...
/*  {"table":"ocus_cart","nf_old_table_fields":[],"nf_new_table_fields":{"1":"api_id"}},*/
    $sql = "REPLACE INTO ocus_cart (`cart_id`, `api_id`, `customer_id`, `session_id`, `product_id`, `recurring_id`, `option`, `quantity`, `date_added`) SELECT 
  `cart_id`, 0 , `customer_id`, `session_id`, `product_id`, `recurring_id`, `option`, `quantity`, `date_added` 
  FROM `".OLD_DB_DATABASE."`.ocus_cart" ;
//    print_r($sql);
    $new_db->query($sql) or die("\n Error replacing into ocus_cart " . mysqli_error($new_db));
...
*/
   foreach ($identical_tables as $table){
       if(!in_array($table, $exception_tables)){
           $result = $new_db->query("SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA ='"
           ...
           if ($result->num_rows >0 ) {
               ...
               while ($row = $result->fetch_assoc()) {
                   if (strstr($row['COLUMN_NAME'],"date")){
                       $columns2[]= "IF (".$row['COLUMN_NAME']." LIKE '0000%', '1970-01-01 00:00:01', ".$row['COLUMN_NAME'].")";
                   } else
               ...
               $sql = "REPLACE INTO $table ($columnList1) SELECT $columnList2  FROM `" . OLD_DB_DATABASE . "`." . $table;
               print_r($sql); echo "\n";

               $new_db->query($sql) or die("\n Error replacing into " . $table . " -> " . mysqli_error($new_db));
               echo "Records in $table replaced successfuly\n";
           } else {
