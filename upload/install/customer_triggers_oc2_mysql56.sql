-- ============================================
-- Customer Sync Triggers for OC2 - MySQL 5.6 Compatible
-- Syncs customer data from OC2 to OC3
  ...
  IF should_sync = 1 THEN
    SET json_data = CONCAT(
      '{',
        ...
        '"firstname":"', REPLACE(REPLACE(IFNULL(NEW.firstname, ''), '\\', '\\\\'), '"', '\\"'), '",',
        '"lastname":"', REPLACE(REPLACE(IFNULL(NEW.lastname, ''), '\\', '\\\\'), '"', '\\"'), '",',
...
SELECT 'Customer triggers (INSERT, UPDATE) installed successfully for OC2 (MySQL 5.6)' AS status;
