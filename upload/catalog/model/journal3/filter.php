...
class ModelJournal3Filter extends Model {
	...
	public function getFilterUrl() {
		switch ($this->journal3_request->get('route')) {
			case 'product/catalog':
				return $this->journal3_url->link('product/catalog', '', true);

			case 'product/category':
				return $this->journal3_url->link('product/category', 'path=' . $this->journal3_request->get('path'), true);

			case 'product/manufacturer' . JOURNAL3_ROUTE_SEPARATOR . 'info':
				return $this->journal3_url->link('product/manufacturer' . JOURNAL3_ROUTE_SEPARATOR . 'info', 'manufacturer_id=' . $this->journal3_request->get('manufacturer_id'), true);
			...
			case 'product/search':
				...
				return $this->journal3_url->link('product/search', $url, true);

			case 'product/special':
				return $this->journal3_url->link('product/special', '', true);
	...
	public function getFilterData() {
		$separator = $this->journal3->get('filterUrlValuesSeparator') ?: ',';
		...
		// specific filter data based on route
		switch ($this->journal3_request->get('route')) {
			case 'product/category':
				$path = explode('_', (string)$this->journal3_request->get('path', ''));
				$filter_data['filter_category_id'] = (int)array_pop($path);

				break;

			case 'product/manufacturer' . JOURNAL3_ROUTE_SEPARATOR . 'info':
				$filter_data['filter_manufacturer_id'] = (int)$this->journal3_request->get('manufacturer_id', '');

				break;

			case 'product/search':
				$filter_data['filter_name'] = $this->journal3_request->get('search', '');
				$filter_data['filter_tag'] = $this->journal3_request->get('tag', $filter_data['filter_name']);
				$filter_data['filter_description'] = $this->journal3_request->get('description', '');
				$filter_data['filter_category_id'] = $this->journal3_request->get('category_id', '');
				$filter_data['filter_sub_category'] = $this->journal3_request->get('sub_category', '');
		...
		if (!empty($this->request->get['sort'])) {
			...
			$filter_data['sort'] = $this->journal3->get('productSort');
		}

		if (!empty($this->request->get['order'])) {
			$filter_data['order'] = $this->request->get['order'];
		} else {
			$filter_data['order'] = $this->journal3->get('productOrder');
		}
		...
		if (!empty($this->request->get['limit'])) {
			$limit = max(1, (int)$this->request->get['limit']);
		} else {
			$limit = max(1, (int)$this->config->get('theme_journal3_product_limit'));
		}
	...
	public function getFilterParams($filter_data) {
		$separator = $this->journal3->get('filterUrlValuesSeparator') ?: ',';
	...
	public function getPriceRange($filter_data) {
		$discount = "
			(
				SELECT price 
				...
				WHERE 
					...
					AND pd2.customer_group_id = '" . (int)$this->journal3_opencart->customer_group_id . "' 
					AND pd2.quantity = '1' 
		...
		if (version_compare(VERSION, '4.1.0.0', '>=')) {
			$sql = "
				SELECT
					MIN(COALESCE(" . $discount . ", p.price)) AS min,
					MAX(COALESCE(" . $discount . ", p.price)) AS max
			...
			$special = "
				(
					...
					WHERE 
						...
						AND ps.customer_group_id = '" . (int)$this->journal3_opencart->customer_group_id . "' 
		...
		$sql .= $this->addFilters($filter_data, 'price');
	...
	public function getCategories($filter_data) {
		$sql = "
			...
			LEFT JOIN `{$this->journal3_db->prefix('category_description')}` cd ON (c.category_id = cd.category_id) 
			...
			LEFT JOIN `{$this->journal3_db->prefix('product')}` p ON (p.product_id = p2c.product_id)
		";

		$sql .= $this->addFilters($filter_data, 'category');

		$sql .= " 
			AND cd.language_id = '" . (int)$this->journal3_opencart->language_id . "' 
			AND c2s.store_id = '" . (int)$this->journal3_opencart->store_id . "' 
			AND c.status = '1'
		";

		if (is_numeric($filter_category_id = Arr::get($filter_data, 'filter_category_id'))) {
	...
	public function getManufacturers($filter_data) {
		if (Arr::get($filter_data, 'filter_manufacturer_id')) {
			return array();
		}
		...
		$sql .= $this->addFilters($filter_data, 'manufacturer');

		$sql .= " AND m2s.store_id = '" . (int)$this->journal3_opencart->store_id . "'";
	...
	public function getAttributes($filter_data) {
		...
		$sql .= "
				AND pa.language_id = '" . (int)$this->journal3_opencart->language_id . "' 
				AND ad.language_id = '" . (int)$this->journal3_opencart->language_id . "' 
		...
		foreach ($query->rows as $row) {
			if (!isset($results[$row['attribute_id']])) {
				$results[$row['attribute_id']] = [
					'attribute_id'   => $row['attribute_id'],
					'attribute_name' => $row['attribute_name'],
					'values'         => [],
				];
			}
			$results[$row['attribute_id']]['values'][$row['value']] = [
				'id'    => $row['value'],
				'value' => $row['value'],
				'total' => $row['total'],
			];
		...
		if (isset($filter_data['attributes'])) {
			foreach ($filter_data['attributes'] as $attribute_id => $attribute) {
				$current_filter_data = $filter_data;
				unset($current_filter_data['attributes'][$attribute_id]);
				...
				$sql .= $this->addFilters($current_filter_data, 'ATTRIBUTES');
	...
	public function getOptions($filter_data) {
		...
		$sql .= "
				AND od.language_id = '" . (int)$this->journal3_opencart->language_id . "' 
				AND ovd.language_id = '" . (int)$this->journal3_opencart->language_id . "'
		";

		if ($this->journal3->get('filterCheckOptionsQuantity')) {
		...
		if (isset($filter_data['options'])) {
			foreach ($filter_data['options'] as $option_id => $option) {
				$current_filter_data = $filter_data;
				unset($current_filter_data['options'][$option_id]);
				...
				$sql .= $this->addFilters($current_filter_data, 'options');
	...
	public function getFilters($filter_data, $opts = []) {
		$sql = "
			SELECT
				f.filter_id id,
				fd.name filter_name,
		...
		if (($filter_category_id = (int)Arr::get($filter_data, 'filter_category_id')) && Arr::get($opts, 'filtersCategoryCheck')) {
			$sql .= "
		...
		$sql .= "
				AND fd.language_id = '" . (int)$this->journal3_opencart->language_id . "' 
				AND fgd.language_id = '" . (int)$this->journal3_opencart->language_id . "'
		...
		if (isset($filter_data['filters'])) {
			foreach ($filter_data['filters'] as $filter_id => $filter) {
				$current_filter_data = $filter_data;
				unset($current_filter_data['filters'][$filter_id]);
				...
				if (($filter_category_id = (int)Arr::get($current_filter_data, 'filter_category_id')) && Arr::get($opts, 'filtersCategoryCheck')) {
					$sql .= "
						LEFT JOIN `" . DB_PREFIX . "category_filter` cf ON (f.filter_id = cf.filter_id)
					";
				}

				$sql .= $this->addFilters($current_filter_data, 'filters');
	...
	public function getTags($filter_data) {
		...
		$sql .= $this->addFilters($filter_data, 'tag');
	...
	public function getAvailability($filter_data) {
		$sql = "
			SELECT 
		...
		$sql .= $this->addFilters($filter_data, 'availability');
	...
	public function getProducts($filter_data) {
		...
		if (Arr::get($filter_data, 'sort') === 'sales') {
			$bestseller = new \Journal3\Opencart\Bestseller($this->registry);
			$bestseller->init();

			$sql .= "
				, sales
				FROM `" . DB_PREFIX . "journal3_product_sales` j3ps
				LEFT JOIN `" . DB_PREFIX . "product` p ON (p.product_id = j3ps.product_id)
			";
		} else {
		...
		if ($this->journal3->get('filterOrderByStock')) {
			$sql .= " in_stock desc, ";
		}
	...
	private function addFilters($filter_data, $query = null) {
		...
		$sql .= " 
			...
			WHERE 
				...
				AND p2s.store_id = '" . (int)$this->journal3_opencart->store_id . "'
				AND pd.language_id = '" . (int)$this->journal3_opencart->language_id . "'
		";
		...
		if (Arr::get($filter_data, 'special')) {
			if (version_compare(VERSION, '4.1.0.0', '>=')) {
				$sql .= " AND EXISTS (SELECT product_id FROM `" . DB_PREFIX . "product_discount` ps WHERE ps.product_id = p.product_id AND ps.customer_group_id = '" . (int)$this->journal3_opencart->customer_group_id . "' AND ps.special = '1' AND ((ps.date_start = '0000-00-00' OR ps.date_start < NOW()) AND (ps.date_end = '0000-00-00' OR ps.date_end > NOW())))";
			} else {
				$sql .= " AND EXISTS (SELECT product_id FROM `" . DB_PREFIX . "product_special` ps WHERE ps.product_id = p.product_id AND ps.customer_group_id = '" . (int)$this->journal3_opencart->customer_group_id . "' AND ((ps.date_start = '0000-00-00' OR ps.date_start < NOW()) AND (ps.date_end = '0000-00-00' OR ps.date_end > NOW())))";
			}
		}
		...
		if ($query !== 'price') {
			...
			if (is_numeric(Arr::get($filter_data, 'price.min')) && is_numeric(Arr::get($filter_data, 'price.max'))) {
				$sql .= " AND COALESCE(" . $special . ", " . $discount . ", p.price) BETWEEN " . (float)$this->undoTax($filter_data, 'min') . " AND " . (float)$this->undoTax($filter_data, 'max') . "";
			} else if (is_numeric(Arr::get($filter_data, 'price.min'))) {
				$sql .= " AND COALESCE(" . $special . ", " . $discount . ", p.price) >= " . (float)$this->undoTax($filter_data, 'min');
	...
	private function applyTax($price) {
		if ($this->journal3->get('filterTaxClassId')) {
			$price = $this->tax->calculate($price, $this->journal3->get('filterTaxClassId'));
		}

		return $price * $this->currency->getValue($this->session->data['currency']);
	}

	private function undoTax($filter_data, $type) {
		$price = $filter_data['price'][$type] ?? null;

		if (empty($filter_data['is_filter']) || !$price) {
			return $price;
		}
		...
		if ($this->journal3->get('filterTaxClassId')) {
			$tax_rates = $this->tax->getRates($price, $this->journal3->get('filterTaxClassId'));
...
class_alias('ModelJournal3Filter', '\Opencart\Catalog\Model\Journal3\Filter');
