...
class ModelJournal3Checkout extends Model {
	...
	public function init() {
		if ($this->customer->isLogged()) {
			...
			$this->session->data['account'] = $this->journal3->get('quickCheckoutAuthentication');
		...
		$this->session->data['same_address'] = $this->journal3->get('quickCheckoutSameAddress');
		$this->session->data['newsletter'] = $this->journal3->get('quickCheckoutNewsConfirmNewsletterChecked');
	...
	private function set_address($type, $address) {
		if (empty($this->session->data[$type . '_address'])) {
			$this->session->data[$type . '_address'] = [];
		}
		...
		if (!Arr::get($this->session->data, $type . '_address.country_id')) {
			switch ($this->journal3->get('quickCheckoutAddressDefaultCountry')) {
				case 'store_address':
			...
			switch ($this->journal3->get('quickCheckoutAddressDefaultZone')) {
				case 'store_address':
	...
	private function shipping_method() {
		...
		if ($code === null && $this->journal3->get('sectionShippingAutoSelect')) {
	...
	private function payment_method() {
		...
		if ($code === null && $this->journal3->get('sectionPaymentAutoSelect')) {
	...
	public function setCheckoutId() {
		$this->session->data['journal3_checkout_id'] = md5(time());

		return $this->session->data['journal3_checkout_id'];
	}

	public function checkCheckoutId($checkout_id) {
		return Arr::get($this->session->data, 'journal3_checkout_id') === $checkout_id;
	}

}

class_alias('ModelJournal3Checkout', '\Opencart\Catalog\Model\Journal3\Checkout');
