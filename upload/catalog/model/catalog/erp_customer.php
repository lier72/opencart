        ...
        foreach ($data as $customer_data) {
            ...
            if ($id_customer[0] <= 0) {
                ...
                $key                        = array(
                    ...
                    'email' => new xmlrpcval(html_entity_decode($customer_data['email']), "string"),
                    'is_company' => new xmlrpcval(true, "boolean"),
                    'phone'=>new xmlrpcval($customer_data['telephone'],"string"),
                );
                if ($id_customer[0] == 0) {
                  $context = array(
                  ...
                  if (!$resp->faultCode()) {
                      ...
                      $add_to_odoo = $this->addto_openerp_merge($val['int'], $customer_data['customer_id'], '-', $userId, $client, $db, $pwd);
        ...
        foreach ($data as $address_data) {
            ...
            if ($search_address[0] <= 0) {
                ...
                if ($address_data['zone_id'] != null && $address_data['zone_id'] != "" && $address_data['zone_id'] != 0) {
                    ...
                    $address1 = html_entity_decode($address_data['address_1']);
                    $address2 = html_entity_decode($address_data['address_2']);
                    ...
                    if ($search_address[0] == 0) {
                        ...
                        if (!$resp->faultCode()) {
                            ...
                            $add_to_odoo = $this->addto_openerp_merge($val['int'], $address_data['customer_id'], $address_data['address_id'], $userId, $client, $db, $pwd);
                            if ($add_to_odoo['is_error'] == 1) {
                                $is_error = 1;
                                $error_message .= $add_to_odoo['error_message'] . ',';
