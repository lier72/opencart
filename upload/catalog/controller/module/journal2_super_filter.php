        ...
        if (version_compare(VERSION, '1.5.5', '>=')) {
            ...
            foreach ($results as $key => $result) {
                ...
                $this->data['filter'] = array(
                    ...
                    'display_mode'      => $display_mode,
                    'type'              => Journal2Utils::getProperty($module_data, 'filters_type.' . $key, 'single'),
                    'values'            => $values,
                );
...
// this is Max's addition to show retail price. Only define reatil_price when user is logged now I define this case
// when retail_price is not equal to price 
      			if ($this->customer->isLogged() && $this->config->get('config_customer_group_id') != $this->config->get('customer_default_group_id')){
	     			$retail_price = $this->currency->format($this->tax->calculate($result['retail_price'], $result['tax_class_id'], $this->config->get('config_tax')));
		    	} else {
			    	$retail_price = false;
			    }
// the end of modifications
            ...
            $this->data['products'][] = array(
                ...
                'thumb2'      => $image2,
                'name'        => $result['name'],
// Max modifications to show retail price
