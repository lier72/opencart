...
class ControllerModuleBlogPopular extends Controller {
	public function index($setting) {
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/module/blog_popular/'.$setting['template'])) {
			return $this->load->view($this->config->get('config_template') . '/template/module/blog_popular/'.$setting['template'], $data);
		} else {
			return $this->load->view('default/template/module/blog_popular/'.$setting['template'], $data);
		}
	}
}
?>