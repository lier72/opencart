...
class ControllerModuleProductBlocks extends Controller {
	public function index($setting) {
		...
		if($status) {
     		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/module/product_blocks.tpl')) {
     			return $this->load->view($this->config->get('config_template') . '/template/module/product_blocks.tpl', $data);
     		} else {
     			return $this->load->view('default/template/module/product_blocks.tpl', $data);
     		}
		}
	}
}
