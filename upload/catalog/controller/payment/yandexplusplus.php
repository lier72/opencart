...
class ControllerPaymentyandexplusplus extends Controller {
	public function index() {
		...
		$data['instructionat'] = $this->config->get('yandexplusplus_instruction_attach');
		$data['btnlater'] = $this->config->get('yandexplusplus_button_later');
		...
		if ($this->config->get('yandexplusplus_instruction_attach')){
			...
			$instros = explode('$', ($this->config->get('yandexplusplus_instruction_' . $this->config->get('config_language_id'))));
			$instroz = "";
			foreach ($instros as $instro) {
				...
				$data['yandexplusplusi'] = $instroz;
		}

		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/yandexplusplus.tpl')) {
            return $this->load->view($this->config->get('config_template') . '/template/payment/yandexplusplus.tpl', $data);
		} else {
            return $this->load->view('default/template/payment/yandexplusplus.tpl', $data);
        }		 
	}
	
	public function confirm() {
		if ($this->config->get('yandexplusplus_createorder_or_notcreate')){ exit(); }
  		$this->language->load('payment/yandexplusplus');
		$this->load->model('checkout/order');
			if ($this->config->get('yandexplusplus_mail_instruction_attach')){
		    	...
		    	$this->model_checkout_order->addOrderHistory($this->session->data['order_id'], $this->config->get('yandexplusplus_on_status_id'), $comment, true);
	    	}
	    	else{
				$this->model_checkout_order->addOrderHistory($this->session->data['order_id'], $this->config->get('yandexplusplus_on_status_id'), true);
			}
	}
}
?>