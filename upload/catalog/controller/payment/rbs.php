...
class ControllerPaymentRbs extends Controller {
    ...
    public function index() {
		...
		$data['bank'] = nl2br($this->config->get('rbs_info_text' . $this->config->get('config_language_id')));
    ...
    {
		// MS correction see below
		$itemCode = 0;
		// end of MS correction
		...
		// because we could have same products in our Cart with same OpenCart product_id we add itemCode
		// so $product_data['itemCode'] would be unique within the Cart
		// Maxim Surdu correction
		$itemCode = $itemCode + 1;
		// end of MS correction
            ...
            $product_data[] = array(
                ...
                'itemCode' => $product['product_id'] . "_" . $itemCode,
        ...
        if (isset($this->session->data['shipping_method']['cost']) && $this->session->data['shipping_method']['cost'] > 0) {
            ...
            $delivery['itemAmount'] = round ($this->session->data['shipping_method']['cost'] * 100) ;
            ...
            $delivery['itemPrice'] = round ($this->session->data['shipping_method']['cost'] * 100);
