...
class ControllerBlogArticle extends Controller {
    ...
    public function __construct($registry) {
        ...
        if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/css/blog/article.css')) {
            $this->document->addStyle('catalog/view/theme/' . $this->config->get('config_template') . '/css/blog/article.css');
        }
    ...
    public function index()
    {
        ...
        if(isset($this->request->get['article_id'])){
            ...
            if(!empty($results)){
                foreach ($results as $product_id) {
                    ...
                    $data['products'][] = array(
                        ...
                        'thumb'       => $image,
                        'name'        => $result['name'],
                        'description' => utf8_substr(html_entity_decode($result['description'], ENT_QUOTES, 'UTF-8'), 0, $this->config->get('config_product_description_length')) . '...',
                        'price'       => $price,
                        'special'     => $special,
                        'tax'         => $tax,
            ...
            if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/blog/article_detail/' . $this->settings['article_detail_template'])) {
                $this->response->setOutput($this->load->view($this->config->get('config_template') . '/template/blog/article_detail/' . $this->settings['article_detail_template'], $data));
            } else {
                $this->response->setOutput($this->load->view('default/template/blog/article_detail/' . $this->settings['article_detail_template'], $data));
            }
