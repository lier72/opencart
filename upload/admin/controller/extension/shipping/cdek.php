...
class ControllerExtensionShippingCdek extends Controller {
	...
	public function index() {
		...
		*/
		...
		$data['update_city'] = $this->url->link('extension/module/cdek_integrator/updateCities', 'user_token=' . $this->session->data['user_token'] . '&redirect=extension/shipping/cdek', true);
		...
		if (isset($this->request->post['shipping_cdek_default_param'])) {
			$data['shipping_cdek_default_param'] = $this->request->post['shipping_cdek_default_param'];
		} else {
			$data['shipping_cdek_default_param'] = $this->config->get('shipping_cdek_default_param');
		}
	...
	private function validate()
	{
		...
		if (!$this->user->hasPermission('modify', 'extension/shipping/cdek')) {
			...
			if ($this->request->post['shipping_cdek_default_param']['use']) {

				$default_param = $this->request->post['shipping_cdek_default_param'];

                foreach (array('size_a', 'size_b', 'size_c') as $item) {

                    if (!is_numeric($default_param[$item])) {
                        $this->error['shipping_cdek_default_param']['size'] = $this->language->get('error_numeric');
                        break;
                    } elseif ($default_param[$item] <= 0) {
                        $this->error['shipping_cdek_default_param']['size'] = $this->language->get('error_positive_numeric');
                        break;
                    }
                ...
                if (!is_numeric($default_param['weight'])) {
                    $this->error['shipping_cdek_default_param']['weight'] = $this->language->get('error_numeric');
                } elseif ($default_param['weight'] <= 0) {
                    $this->error['shipping_cdek_default_param']['weight'] = $this->language->get('error_positive_numeric');
                }
