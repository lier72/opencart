...
class ControllerExtensionFeedYandexSitemap extends Controller {
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			$this->model_setting_setting->editSetting('feed_yandex_sitemap', $this->request->post);
		...
		if (isset($this->request->post['feed_yandex_sitemap_status'])) {
			$data['feed_yandex_sitemap_status'] = $this->request->post['feed_yandex_sitemap_status'];
		} else {
			$data['feed_yandex_sitemap_status'] = $this->config->get('feed_yandex_sitemap_status');
		}

		$data['data_feed'] = HTTPS_CATALOG . 'index.php?route=extension/feed/yandex_sitemap';
