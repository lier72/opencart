...
class ControllerModuleCdekIntegrator extends Controller {
	...
	public function index() {
		...
		if (!$this->new_application) {
			...
			$new_orders = $this->model_module_cdek_integrator->getTotalOrders($exdata);
		...
		$results = $this->model_module_cdek_integrator->getDispatchList($exdata);
		...
		$rdata['dispatch_list'] = $this->url->link('module/cdek_integrator/dispatch', 'token=' . $this->session->data['token'], 'SSL');
		
		$rdata['order'] = $this->url->link('module/cdek_integrator/order', 'token=' . $this->session->data['token'], 'SSL');
		$rdata['option'] = $this->url->link('module/cdek_integrator/option', 'token=' . $this->session->data['token'], 'SSL');
        $rdata['update_city'] = $this->url->link('module/cdek_integrator/updateCities', 'token=' . $this->session->data['token'], true);
        $rdata['cancel'] = $this->url->link('module', 'token=' . $this->session->data['token'] . '&type=module', true);
		...
		$this->response->setOutput($this->load->view('module/cdek_integrator/cdek_integrator.tpl', $rdata));
	}
	...
	private function orderList() {
		...
		if ($this->new_application) {
			$this->response->redirect($this->url->link('module/cdek_integrator', 'token=' . $this->session->data['token'], 'SSL'));
		}
		...
		$rdata['create'] = html_entity_decode($this->url->link('module/cdek_integrator/createOrder', 'token=' . $this->session->data['token'], 'SSL'), ENT_QUOTES, 'UTF-8');
		$rdata['cancel'] = $this->url->link('module/cdek_integrator', 'token=' . $this->session->data['token'], 'SSL');
		...
		$results = $this->model_module_cdek_integrator->getOrders($exdata);

		$order_total = $this->model_module_cdek_integrator->getTotalOrders($exdata);
		...
		$rdata['sort_order'] = $this->url->link('module/cdek_integrator/order', 'token=' . $this->session->data['token'] . '&sort=o.order_id' . $url, 'SSL');
		$rdata['sort_customer'] = $this->url->link('module/cdek_integrator/order', 'token=' . $this->session->data['token'] . '&sort=customer' . $url, 'SSL');
		$rdata['sort_status'] = $this->url->link('module/cdek_integrator/order', 'token=' . $this->session->data['token'] . '&sort=status' . $url, 'SSL');
		$rdata['sort_total'] = $this->url->link('module/cdek_integrator/order', 'token=' . $this->session->data['token'] . '&sort=o.total' . $url, 'SSL');
		$rdata['sort_date_added'] = $this->url->link('module/cdek_integrator/order', 'token=' . $this->session->data['token'] . '&sort=o.date_added' . $url, 'SSL');
		...
		foreach ($this->limits as $item) {
			$rdata['limits'][$item] = $this->url->link('module/cdek_integrator/order', 'token=' . $this->session->data['token'] . '&limit=' . $item . $url, 'SSL');
		}
		...
		$this->response->setOutput($this->load->view('module/cdek_integrator/order_list.tpl', $rdata));
	}
	
	public function createOrder() {
		
		if (empty($this->request->get['orders']) || !array_filter($this->request->get['orders'])) {
			$this->response->redirect($this->url->link('module/cdek_integrator/order', 'token=' . $this->session->data['token'], 'SSL'));
		}
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateOrderFrom()) {
			...
			if (count($cdek_orders)) {
				...
				$this->model_module_cdek_integrator->addDispatch($exdata);
				...
				if ($all) {
					...
					$this->response->redirect($this->url->link('module/cdek_integrator/dispatch', 'token=' . $this->session->data['token'] . $url, 'SSL'));
				}
        ...
        $rdata['entry_pvz_sell'] = $this->language->get('entry_pvz_sell');
        ...
        $rdata['entry_sell_address'] = $this->language->get('entry_sell_address');
		...
		$rdata['token'] = $this->session->data['token'];
		$rdata['ajax_url'] = $this->getInfo()->getAjaxUrl();
		...
		$rdata['action'] = $this->url->link('module/cdek_integrator/createOrder', 'token=' . $this->session->data['token'] . $url, 'SSL');
		$rdata['cancel'] = $this->url->link('module/cdek_integrator/order', 'token=' . $this->session->data['token'], 'SSL');
		...
		$additional_cost_totals = ['shipping', 'cod_cdek_total'];
		...
		foreach ($this->request->get['orders'] as $order_id) 
		{
			...
			if ($order_info && !$this->model_module_cdek_integrator->orderExists($order_id)) {
				...
				foreach ($totals as $totals_key => $totals_value) {
					$totals[$totals_key]['text'] = $this->currency->format($totals_value['value']);
		...
		$this->response->setOutput($this->load->view('module/cdek_integrator/order_form.tpl', $rdata));
	}
	...
	public function option() {
		...
		$rdata['action'] = $this->url->link('module/cdek_integrator/option', 'token=' . $this->session->data['token'], 'SSL');
		$rdata['cancel'] = $this->url->link('module/cdek_integrator', 'token=' . $this->session->data['token'], 'SSL');
		...
		$this->response->setOutput($this->load->view('module/cdek_integrator/option.tpl', $rdata));
	}
	...
	private function dispatchList() {
		...
		$order_total = $this->model_module_cdek_integrator->getDispatchTotal($exdata);
		...
		foreach ($results as $dispatch_info) {
			...
			$rdata['dispatches'][] = array(
				...
				'status'				=> $dispatch_info['status_description'],
				'status_date'			=> $this->formatDate($dispatch_info['status_date']),
				'cost'					=> (float)$dispatch_info['delivery_cost'] ? $this->currency->format($dispatch_info['delivery_cost']) : '',
				'action'				=> $action
			);
		...
		$rdata['sort_order_id'] = $this->url->link('module/cdek_integrator/dispatch', 'token=' . $this->session->data['token'] . '&sort=o.order_id' . $url, 'SSL');
		$rdata['sort_dispatch_number'] = $this->url->link('module/cdek_integrator/dispatch', 'token=' . $this->session->data['token'] . '&sort=d.dispatch_number' . $url, 'SSL');
		$rdata['sort_act_number'] = $this->url->link('module/cdek_integrator/dispatch', 'token=' . $this->session->data['token'] . '&sort=o.act_number' . $url, 'SSL');
		$rdata['sort_date'] = $this->url->link('module/cdek_integrator/dispatch', 'token=' . $this->session->data['token'] . '&sort=d.date' . $url, 'SSL');
		$rdata['sort_city_from'] = $this->url->link('module/cdek_integrator/dispatch', 'token=' . $this->session->data['token'] . '&sort=o.city_name' . $url, 'SSL');
		$rdata['sort_city_to'] = $this->url->link('module/cdek_integrator/dispatch', 'token=' . $this->session->data['token'] . '&sort=o.recipient_city_name' . $url, 'SSL');
		$rdata['sort_status'] = $this->url->link('module/cdek_integrator/dispatch', 'token=' . $this->session->data['token'] . '&sort=o.status_id' . $url, 'SSL');
		$rdata['sort_total'] = $this->url->link('module/cdek_integrator/dispatch', 'token=' . $this->session->data['token'] . '&sort=o.delivery_cost' . $url, 'SSL');
		...
		foreach ($this->limits as $item) {
			$rdata['limits'][$item] = $this->url->link('module/cdek_integrator/dispatch', 'token=' . $this->session->data['token'] . '&limit=' . $item . $url, 'SSL');
		}
		...
		$this->response->setOutput($this->load->view('module/cdek_integrator/dispatch_list.tpl', $rdata));
	}
	...
	public function dispatchView() {
		...
		if ($this->new_application) {
			...
			$this->response->redirect($this->url->link('module/cdek_integrator/dispatch', 'token=' . $this->session->data['token'], 'SSL'));
		}
		...
		if ($dispatch_info) {
			...
			$rdata['sync'] = $this->url->link('module/cdek_integrator/dispatchSync', 'token=' . $this->session->data['token'] . '&order_id=' . $this->request->get['order_id'], 'SSL');
			$rdata['print'] = $this->url->link('module/cdek_integrator/dispatchPrint', 'token=' . $this->session->data['token'] . '&order_id=' . $this->request->get['order_id'], 'SSL');
			$rdata['cancel'] = $this->url->link('module/cdek_integrator/dispatch', 'token=' . $this->session->data['token'], 'SSL');
			...
			if ((float)$dispatch_info['delivery_cost']) {
				$rdata['delivery_cost'] = $this->currency->format($dispatch_info['delivery_cost']);
			} else {
				$rdata['delivery_cost'] = 0;
			}
			
			if ((float)$dispatch_info['delivery_recipient_cost']) {
				$rdata['delivery_recipient_cost'] = $this->currency->format($dispatch_info['delivery_recipient_cost']);
			}
			...
			if (file_exists(DIR_DOWNLOAD . 'cdek/order-' . $this->request->get['order_id'] . '.pdf') && is_file(DIR_DOWNLOAD . 'cdek/order-' . $this->request->get['order_id'] . '.pdf')) {
				$rdata['pdf'] = $this->url->link('module/cdek_integrator/showPdf', 'token=' . $this->session->data['token'].'&order_id='.$this->request->get['order_id'], 'SSL');
			}
			...
			if ($rdata['add_service_total']) {
				$rdata['add_service_total'] = $this->currency->format($rdata['add_service_total']);
			}
		...
		if($this->isAjax()) {
			$this->response->setOutput($this->load->view('module/cdek_integrator/dispatch_info.tpl', $rdata));
		} else {
			$rdata['header'] = $this->load->controller('common/header');
			$rdata['column_left'] = $this->load->controller('common/column_left');
			$rdata['footer'] = $this->load->controller('common/footer');

			$this->response->setOutput($this->load->view('module/cdek_integrator/dispatch_info_full.tpl', $rdata));
		}
	}
	...
	public function dispatchPrint() 
	{
		...
		if (file_exists(DIR_DOWNLOAD . 'cdek/order-' . $this->request->get['order_id'] . '.pdf')) 
		{
			if ($this->isAjax()) 
			{
				...
				$json['file'] = $this->url->link('module/cdek_integrator/showPdf', 'token=' . $this->session->data['token'].'&order_id='.$this->request->get['order_id'], 'SSL');
				$this->response->setOutput(json_encode($json));
				exit;
			} else {
				$this->response->redirect($this->url->link('module/cdek_integrator/dispatchView', 'token=' . $this->session->data['token'] . '&order_id=' . $this->request->get['order_id'], 'SSL'));
			}
		}
	...
	private function sync($orders = array()) {
		...
		if (!empty($this->api->error)) {
// this line was added but has not been tested yet
		    $this->log->write($this->api->error);
			return FALSE;
		}
	...
	public function dispatchSync() {
		...
		if ($dispatch_info) {
			...
			if (!empty($this->api->error)) {
				...
				$this->model_module_cdek_integrator->editDispatch($this->request->get['order_id'], $exdata);
		...
		if ($this->isAjax()) {
			...
			if (!empty($this->session->data['warning'])) {
				...
				if (isset($this->request->get['target']) && $this->request->get['target'] == 'list') {
					...
					$this->load->controller('module/cdek_integrator/dispatchView');
				}
	...
	private function init() {
		...
		$this->setting['edit_mode'] = $this->user->hasPermission('modify', 'module/cdek_integrator');
	...
	public function testapi() {
		$this->load->model('module/cdek_integrator');
		$this->model_module_cdek_integrator->statusApi(21, 8, 0, 'test model');
	}
	...
	public function cron()
	{
		...
		$_dispatches = $this->model_module_cdek_integrator->getDispatchesToSync(false);
		...
		if (!$orders) {
			echo "Нет заказов для проверки."; return;
		}
		...
		if (!isset($status->Order)) {
			echo "ОШИБКА: не получены статусы."; return;
		}
		...
		foreach ($status->Order as $key => $order) 
		{
			...
			if(!isset($dispatches[$dispatch_number])) {
				echo "ВНИМАНИЕ: нет заказа ".$dispatch_number."<BR>"; 
				continue;
			}
			...
			if((int)$dispatch['status_id'] == (int)$status_id) {
				echo "Заказ ".$dispatch_number." не изменен<BR>"; 
				continue;
			}

			echo "Обработка ".$dispatch_number." - ".$status_id."<BR>"; 
			...
			{
				$this->model_module_cdek_integrator->editDispatch($dispatch['order_id'], $update[$dispatch['order_id']]);
    ...
    public function getCityByName($cityName = null) {
        ...
        if ($cityName) {
            $rawCities = $this->getInfo()->getCityByName($cityName);
    ...
    public function updateCities() {
        ...
        $this->model_module_cdek_integrator->deleteCities();
        ...
        foreach ($countries as $country) {
            ...
            for ($i = 0; ; $i++) {
                ...
                foreach ($out as $value) {
                    if (!empty($value['code'])) {
                        ...
                        if (!empty($value['payment_limit'])) {
                            $payment_limit = (float)$value['region'];
                        } else {
                ...
                $this->model_module_cdek_integrator->addCities($data);
    ...
    private function templateOutput($tpl, $data) {
        return $this->load->view('module/cdek_integrator/'.$tpl, $data);
    }
}
